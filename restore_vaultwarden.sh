...
# 设置文件锁
LOCK_FILE="/var/run/vaultwarden-restore.lock"  # 和主脚本一致
exec 200>"$LOCK_FILE"
if ! flock -n 200; then
  echo "$(date '+%F %T') - 【跳过】另一个备份/恢复实例正在运行" | tee -a "$LOG_FILE"
  exit 0
fi
...
# 检查环境配置文件
[[ -f "$BACKUP_ENV" ]] || log_err "备份环境文件 $BACKUP_ENV 不存在"
...
# 检查密钥文件
[[ -f "$SECRETS_FILE" ]] || log_err "SECRETS_FILE $SECRETS_FILE 不存在"

# 获取 restic 密码
RESTIC_PASSWORD=$(jq -r '.RESTIC_PASSWORD' "$SECRETS_FILE")

# 检查数据目录
[[ -d "$DATA_DIR" ]] || log_err "数据目录 $DATA_DIR 不存在"
...
if ! docker run --rm \
  ...
  -e RESTIC_REPOSITORY="s3:$R2_ENDPOINT/$R2_BUCKET" \
  -v "$DATA_DIR":/data:ro \
  "$RESTIC_IMAGE" backup /data --json 2>/tmp/restic-warn.log >/tmp/restic-summary.json; then
...
[[ -z "$SNAP_ID" || "$SNAP_ID" == "null" ]] && log_err "无法提取快照 ID"
...
if ! docker run --rm \
  ...
  -e RESTIC_PASSWORD="$RESTIC_PASSWORD" \
  -e RESTIC_REPOSITORY="s3:$R2_ENDPOINT/$R2_BUCKET" \
  "$RESTIC_IMAGE" check >/dev/null; then
...
if ! docker run --rm \
  ...
  -e RESTIC_PASSWORD="$RESTIC_PASSWORD" \
  -e RESTIC_REPOSITORY="s3:$R2_ENDPOINT/$R2_BUCKET" \
  "$RESTIC_IMAGE" check --read-data-subset=5% >/dev/null 2>&1; then
...
# Vaultwarden 自动备份任务
0 3 * * * root /usr/local/bin/vw-backup >> $LOG_DIR/backup.log 2>&1

# Docker 系统清理任务
0 4 * * * root docker system prune -f --filter 'label!=vaultwarden' >> $LOG_DIR/prune.log 2>&1
EOF
  
  # 设置正确的文件权限
  chmod 0644 /etc/cron.d/vaultwarden-backup
  
  # 不需要重新加载cron，它会自动检测配置更改
  log_info "已配置定时备份和清理任务 (cron 会自动检测配置更改)"
}
...
EOF
  ...
  log_info "配置日志轮转 (/etc/logrotate.d/vaultwarden)"
}
